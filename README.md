...
- **Фактор** (свип) — сетка «параметр × порог» одного фактора → **тепловая карта** условной
  ...
  **накопительно** (`≥/≤ порог`) или **по диапазонам** (непересекающиеся корзины `<0, 0–5, 5–10…`
  — видно вклад каждой зоны отдельно). Клик по ячейке раскрывает затухание по горизонтам,
