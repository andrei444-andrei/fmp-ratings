...
+ свои тикеры) — по умолчанию ничего не выбрано. «Крупные акции» = **актуальный состав S&P 500**
(≈500 ликвидных крупнокапов, динамически из FMP `GET /api/signals/universe`, кэш `signals_universe`,
fallback на статику). Цены в движок грузятся **чанками** (по 40 тикеров) — поэтому большие вселенные
(до ~500) проходят без переполнения памяти Pyodide. Окно анализа ограничивается **годами «от–до»**,
