...
### Heatmap → новости дня (Marketaux + GDELT fallback)
...
2. **Marketaux** (если `MARKETAUX_KEY` задан и месячный счётчик < `MARKETAUX_MONTHLY_CAP`, по умолчанию 8000) — 1 запрос за дату, история с 2017, ~10к запросов/мес на Standard plan ($99).
3. **GDELT** (бесплатный fallback) — если Marketaux не задан, превышен cap, или вернул 0 статей.
4. **AI** (AIML) выбирает 3-5 значимых из реальных заголовков и описывает на русском только фактуру (без «оказало влияние / инвесторы реагируют»).
5. Результат записывается в кэш.

Переменные окружения:
```
AIMLAPI_KEY=...                  # обязательно
MARKETAUX_KEY=...                # опционально; без него используется GDELT
MARKETAUX_MONTHLY_CAP=8000       # опционально; safety cap, после переключаемся на GDELT
```
