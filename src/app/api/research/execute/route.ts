...
const SYS_PROMPT =
  ...
  'КРИТИЧНО про много вызовов: если обогащаешь LLM-ом НЕСКОЛЬКО строк (более ~5) — НЕ делай последовательный цикл `for ...: await ask_ai(...)` (это медленно и упирается в лимит времени сервера ~300с). ' +
  'Вместо этого собери СПИСОК промптов и вызови `answers = await ask_ai_many(prompts, web=True, concurrency=8)` — он выполнит запросы ПАРАЛЛЕЛЬНО (с ограничением одновременности) и вернёт список ответов в ТОМ ЖЕ порядке (ошибки — поштучно, как строка). Это многократно быстрее. ' +
  'Сначала ДЕДУПЛИЦИРУЙ запросы (уникальные страна+период) и при необходимости ограничь число самыми важными случаями. ' +
  ...
  'ПОЭТАПНЫЙ вывод: есть функция emit(x) — она рисует блок в UI СРАЗУ, не дожидаясь конца скрипта (принимает то же, что и result: DataFrame/Styler/kpi/bars/callout/row/dict/list/строку). ' +
  'Для ДЛИННЫХ прогонов (особенно с ask_ai/ask_ai_many) выводи результат по этапам через emit(...) — например emit(итоговую таблицу метрик) сразу, затем собери новости батчем и emit(таблицу с новостями): пользователь видит данные по мере готовности, и при достижении лимита времени уже выведенные блоки сохраняются. ' +
  'ВАЖНО: emit реально «проталкивает» блок в UI на точках await (между батчами ask_ai_many), поэтому ставь тяжёлые await-операции МЕЖДУ emit-ами. Если выводишь всё через emit — оставь result = None (иначе то, что в result, нарисуется ПОВТОРНО в конце). Можно совмещать: emit(...) по ходу + result = финальный итог. ' +
  ...
  'развёрнутый текст в ячейке (например новости) — это нормально: он отрендерится как markdown с переносом и ограничением высоты, но держи его по делу (1–4 предложения на ячейку), не вставляй простыни; ' +
