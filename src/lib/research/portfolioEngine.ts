...
// maxLeverage — макс. плечо (1 = без плеча). Плечо УМНОЖАЕТ загрузку: доля на имя = L·min(1/m, потолок),
// итог = L·min(1, m·потолок) (эффективный потолок на имя = потолок·L). Простаивающий капитал (загрузка <100%)
// паркуется как выбрано (SPY/BIL/кэш); заёмная часть (>100%) финансируется по безрисковой ставке (BIL).
...
// Вес на 1 имя (доля капитала): плечо УМНОЖАЕТ базовый (равновзвешенный, с потолком) вес.
// база на имя = min(1/m, C), где C = maxW (или 1 без лимита); per = L · база; итог = L · min(1, m·C).
// Т.е. плечо L масштабирует всю загрузку (эффективный потолок на имя = C·L), а не только при широкой корзине.
// L=1 → per = min(1/m, C) (как без плеча). Размещённая доля per·m может быть >1 (плечо) или <1 (в паркинг).
const cappedPerWeight = (m: number, maxW: number, L = 1): number => (m <= 0 ? 0 : L * Math.min(1 / m, maxW > 0 ? maxW : 1));
