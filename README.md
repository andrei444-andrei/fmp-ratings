...
3. **Use-кейсы** (вкладки) над выбранными стратегиями: готовы **«Сравнение по годам»** и
   **«Объединённый портфель»** (ребаланс к весам раз в месяц, но эквити отслеживается **по дням** →
   **реальная дневная просадка**; CAGR, итог, vs бенчмарк; `/api/quantconnect/series` отдаёт дневные
   ряды из того же кэша). В дорожной карте — сводка по стратегии, риск/корреляция, анализ просадок.
5. **AI-ассистент** (всплывающий чат на `/quant`, `POST /api/quantconnect/chat`): отвечает на вопросы
   о стратегиях по реальным данным портфеля — сервер подставляет в промпт сводку (доходности,
   просадки, сравнение с SPY), AI отвечает в Markdown через aimlapi.
