...
describe('anchorYearAttribution — якорим к фактической годовой доходности (плечо)', () => {
  ...
  it('реконструкция за год почти сократилась (Σ≈0), факт большой → НЕ раздуваем ячейки (баг 2018)', () => {
    // как 2018 у BUY DEEP: Σ recon ≈ +0.1%, факт стратегии −13.78% → старый масштаб ×(−100) раздувал
    const contrib = { SPY: -0.0264, SMH: 0.0174, TUR: 0.0165, EWY: 0.0032 }; // Σ ≈ +1.07%
    const excess = { SPY: 0, SMH: 0.0113, TUR: 0.020, EWY: 0.0017 };
    const a = anchorYearAttribution(contrib, excess, -0.1378, -0.0457);
    for (const v of Object.values(a.contrib)) expect(Math.abs(v)).toBeLessThan(0.1); // |<10%|, а не +268%
    expect(a.contrib.SPY).toBeCloseTo(-0.0264, 9);        // ячейка как есть (разные знаки real/recon → без масштаба)
    expect(a.totalContrib).toBeCloseTo(-0.1378, 9);       // «Итог» всё равно фактический
    expect(a.totalExcess).toBeCloseTo(-0.1378 - -0.0457, 9);
  });
});
