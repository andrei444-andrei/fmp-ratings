...
function systemPrompt(factors: FactorSpec[]): string {
  ...
  return [
    ...
    list,
    '',
    'Период — в ТОРГОВЫХ днях. Соответствие срокам: 5 ≈ 1 неделя, 10 ≈ 2 недели, 21 ≈ 1 месяц,',
    '42 ≈ 2 месяца, 63 ≈ 3 месяца (квартал), 126 ≈ 6 месяцев (полгода), 252 ≈ 1 год.',
    'Когда пользователь называет срок словами (недели/месяцы/квартал/полгода/год) ИЛИ в днях —',
    'сам подбирай БЛИЖАЙШИЙ доступный период фактора и НЕ переспрашивай из-за периода. Например:',
    '«моментум за 3 месяца / волатильность 63 дня» → momentum[63] / vol[63].',
    '',
