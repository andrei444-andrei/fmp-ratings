...
2. **Модель сигналов** (`/signals`) — детерминированный движок факторной модели: майнинг
   связей (ранг-IC), значимость с FDR-поправкой, событийный анализ базовых сигналов и
   модуляция вторичными факторами, коллинеарность (VIF), веса (Fama-MacBeth / Ridge /
   ElasticNet), walk-forward OOS и live-скоринг инструментов.
...
|---|---|---|---|---|
| **FinancialModelingPrep** | `FMP_API_KEY` | `src/lib/fmp.ts` | `/api/research/execute`, `/api/signals/execute` (через `src/lib/research/*`, `src/lib/signals/*`) | `/research`, `/signals` |
| **aimlapi.com** (LLM-агрегатор, OpenAI-совместимый) | `AIMLAPI_KEY` (+ опц. `AIMLAPI_MODEL`) | `src/lib/aimlapi.ts` | `/api/research/execute`, `/api/research/models`, `/api/signals/execute` (опц. AI-нарратив) | `/research`, `/signals` |
...
Детерминированный (НЕ генерируемый LLM) Python-пайплайн факторной модели, исполняется в том же
Pyodide-движке, что и `/research`, на ценах FMP (с синтетик-fallback без ключа). Конфиг —
вселенная (страновые/секторные/факторные ETF + крупные акции, пресеты + свои тикеры), бенчмарк,
горизонт таргета и уровень FDR. Базовые сигналы (моментум-экстремумы, превышение бенчмарка,
волатильность, близость к ATH) задают режим; вторичные факторы его модулируют.

Стадии отчёта (стримятся по ходу): сводка панели → **одиночные факторы** (ранг-IC, period-clustered
t-стат) → **базовые сигналы** (событийный анализ) → **модуляция** (регрессия вторичных факторов
внутри события) → **коллинеарность** (корреляции + VIF) → **веса** (Fama-MacBeth + Ridge +
ElasticNet) → **walk-forward OOS** (расширяющееся окно, OOS-IC и long-short спред) →
**live-скоринг** (ранжирование инструментов по ожидаемой избыточной доходности на последнюю дату).
Значимость намайненных связей — с поправкой на множественные сравнения (Benjamini-Hochberg, FDR).
Ядро статистики реализовано на numpy/pandas (без sklearn/statsmodels) — быстрый и воспроизводимый
прогон. Опциональный AI-нарратив по методологии — поверх (под `AIMLAPI_KEY`), на ядро не влияет.

Прогоны (конфиг + снимок отчёта) сохраняются в `signal_runs`, поддерживают открытие, правку и
удаление. Сервер: `POST /api/signals/execute` (стрим NDJSON), `GET/POST /api/signals/runs`,
`GET/PATCH/DELETE /api/signals/runs/[id]`. Lib: `src/lib/signals/*`.
...
| `signal_runs` | сохранённые прогоны факторной модели (`/signals`): конфиг + снимок HTML-отчёта |
