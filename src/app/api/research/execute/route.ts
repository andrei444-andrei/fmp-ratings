...
const SYS_PROMPT =
  ...
  'ПОЭТАПНЫЙ вывод (ОБЯЗАТЕЛЬНО, если есть медленное обогащение через ask_ai/ask_ai_many): есть функция emit(x) — она рисует блок в UI СРАЗУ, не дожидаясь конца скрипта (принимает то же, что и result: DataFrame/Styler/kpi/bars/callout/row/dict/list/строку). ' +
