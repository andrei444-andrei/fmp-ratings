...
const SYS_PROMPT =
  ...
  'В широкой таблице (wide=True) ВСЕГДА есть колонка `SPY` (бенчмарк добавляется автоматически), поэтому `px["SPY"]` доступен всегда; при переборе СВОИХ тикеров исключай "SPY" (например `[c for c in px.columns if c != "SPY"]`). ' +
  ...
  'cards(*items) — поставить несколько kpi/бейджей в один ряд (НЕ называй её row — это имя часто занимают переменной цикла); ' +
  ...
  "result = [cards(kpi('CAGR', '11.5%', '+2.1%'), kpi('Макс. просадка', '-41%')), bars(top10, title='Топ по доходности'), df, callout('Часть данных — demo', tone='warn')]. " +
